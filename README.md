...
Реализован класс `Minesweeper` для игры "Сапёр".
...
### Методы
- `open_cell` - аналог клика левой кнопкой мыши
...
- `mark_cell` - аналог клика правой кнопкой мыши
  - если игра выиграна или проиграна, ничего не делает 
  - пустую клетку отмечает флажком
  - с клетки с флажком снимает флажок
- `get_game_status` - возвращает статус игры
- `get_game_time` - возвращает время игры в секундах. Время начинает отсчитываться при первом клике в игре и замораживается при выигрыше или проигрыше. До первого клика в игре возвращает 0.
- `render_field` - возвращает список строк, соответствующий отрисованному полю. Соответствие элементов символам:
  - закрытая клетка - `-`
  - клетка с миной - `*`
  - клетка с флагом - `?`
